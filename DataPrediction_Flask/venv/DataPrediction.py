...
app = Flask(__name__)
r = redis.StrictRedis(host="localhost", port=6377, db=0,password="dlcjf")
# 추후 redis는 localhost -> ip address 변경할 것

'''
POST 타입으로, json으로 저장할 것
[{pID, output, demand, storage},...,{}]
모든 유저( 혹은 최대한 많은 유저 )들에게 받은 후에 전송 받는다.
'''
...
# 예측 데이터를 Redis에 저장, MySQL에는 저장 X
